...
def initialize_capacities():
    """Initialize capacities for energy sources.

    Reads capacities from config.py if it exists, 
    otherwise uses default values.

    Returns:
        tuple: Hydro, solar, wind capacities, 
               battery capacity, initial battery level.
    """
    if os.path.exists('config.py'):
        try:
            ...
            logging.warning(
                "Error importing config.py, using defaults."
            )
...
def simulate_weather():
    ...
    Simulates temperature, wind speed, and solar 
    radiation based on current date and time.

    Returns:
        tuple: Temperature (°C), wind speed (m/s),
...
def simulate_hydroelectricity():
    ...
    logging.debug(
        f"Hydroelectricity simulated: "
        f"hydro_power={hydro_power:.2f} kW"
    )
    return hydro_power
...
def update_data():
    ...
    Simulates weather, energy usage, and 
    generation. Updates battery level, grid 
    usage, and battery health.
    """
    global weather_data, energy_data, battery_level, grid_usage
    global battery_health, battery_age, charge_cycles
    global grid_usage_cost, total_savings
    try:
        ...
        # Calculate savings
        total_savings = (energy_usage - grid_usage) * \
                        GRID_COST_PER_KWH
        ...
        battery_health = max(
            0, battery_health - 
            degradation_rate)  # Decrease battery health over time

        weather_data.append(
            [current_time, temperature, wind_speed, solar_radiation])
        energy_data.append([current_time, energy_usage, hydro_power, 
        ...
        logging.debug(
            f"Data updated: time={current_time}, "
            f"temperature={temperature}, "
            f"wind_speed={wind_speed}, "
            f"solar_radiation={solar_radiation}, "
            f"energy_usage={energy_usage}, "
            f"hydro_power={hydro_power}, "
            f"solar_power={solar_power}, "
            f"wind_power={wind_power}, "
            f"battery_level={battery_level}, "
            f"grid_usage={grid_usage}, "
        ...
        if 'root' in globals():
            root.after(
                ...
                wind_speed, solar_radiation, energy_usage,
...
def animate(_):
    """Update and animate the graph."""
    update_data()

    energy_df = pd.DataFrame(
        energy_data,
        columns=[
            'Time', 'Energy Usage', 'Hydro Power',
            'Solar Power', 'Wind Power', 'Battery Level',
            'Grid Usage', 'Battery Health', 'Grid Usage Cost',
            'Total Savings'
        ])
    ...
    ax1.set_ylim(0,
                 max(energy_df['Hydro Power'].max(), 5000))
    ...
    # Dynamic y-axis limit
    ax2.set_ylim(0,
                 max(energy_df['Solar Power'].max(), 5000))
    ...
    # Dynamic y-axis limit
    ax3.set_ylim(0, max(energy_df['Wind Power'].max(), 5000))
    ...
    # Dynamic y-axis limit
    ax4.set_ylim(0,
                 max(energy_df['Battery Level'].max(), 5000))
    ...
    # Dynamic y-axis limit
    ax5.set_ylim(0, max(energy_df['Grid Usage'].max(), 5000))

    plt.tight_layout()


def update_text_field(current_time, temperature, wind_speed,
                      solar_radiation, energy_usage, hydro_power,
                      solar_power, wind_power, battery_level,
                      grid_usage, battery_health, grid_usage_cost,
                      total_savings):
    """Update the text field with simulation data."""
    ...
    text_field.insert(
        tk.END, f"Grid Usage Cost: £{grid_usage_cost:.2f}\n")
    text_field.insert(
        tk.END, f"Total Savings: £{total_savings:.2f}\n")
    text_field.config(state=tk.DISABLED)
    update_weather_readings(temperature, wind_speed, solar_radiation)
    update_battery_readings(battery_level, battery_health,
                            battery_age, charge_cycles,
                            grid_usage_cost, total_savings)


def update_weather_readings(temperature, wind_speed,
                          solar_radiation):
    """Update weather readings labels."""
...
def update_battery_readings(battery_level, battery_health,
                            battery_age, charge_cycles,
                            grid_usage_cost, total_savings):
    """Update battery readings labels."""
    ...
    battery_age_label.config(
        text=f"Battery Age: {battery_age:.2f} years")
    charge_cycles_label.config(
        text=f"Charge Cycles: {charge_cycles:.2f}")
...
def update_hydro_status(status):
    """Update the hydro power source status."""
    global hydro_active
    hydro_active = status
    hydro_button.config(text=f"Hydro {'ON' if hydro_active else 'OFF'}",
                       bg="green" if hydro_active else "red")
...
def update_solar_status(status):
    """Update the solar power source status."""
    global solar_active
    solar_active = status
    solar_button.config(text=f"Solar {'ON' if solar_active else 'OFF'}",
                       bg="green" if solar_active else "red")
...
def update_wind_status(status):
    """Update the wind power source status."""
    global wind_active
    wind_active = status
    wind_button.config(text=f"Wind {'ON' if wind_active else 'OFF'}",
                       bg="green" if wind_active else "red")
...
# Add input fields for hydro, solar, wind, and
# battery capacities
...
def update_capacities():
    """Update energy source capacities."""
    ...
    try:
        ...
        )  # Ensure battery level doesn't exceed capacity
        update_capacity_display()
        messagebox.showinfo("Success", "Capacities updated!")
        ...
        messagebox.showerror(
            "Error", "Please enter valid numbers for capacities.")
        play_sound()


def update_grid_cost():
    """Update the grid cost per kWh."""
    ...
    try:
        ...
        messagebox.showinfo("Success", "Grid cost updated!")
        play_sound()
    except ValueError as e:
        logging.error(f"Error updating grid cost: {e}")
        messagebox.showerror(
            "Error", "Please enter a valid number for grid cost."
        )
        play_sound()


def update_grid_cost_display():
    """Update the grid cost label."""
    grid_cost_label.config(text=f"£{GRID_COST_PER_KWH:.2f} per kWh")
...
def generate_report(data, title):
    ...
    for entry in data:
        (time, energy_usage, hydro_power, solar_power,
         wind_power, battery_level, grid_usage,
         battery_health, grid_usage_cost,
         total_savings) = entry
...
def generate_daily_report():
    ...
    if not daily_data:
        messagebox.showinfo(
            "Daily Report", "No data for the last 24 hours.")
        play_sound()
        return
    ...
    for hour, entries in sorted(hourly_data.items()):
        avg_energy_usage = sum(entry[1] for entry in entries) \
                           / len(entries)
        avg_grid_usage = sum(entry[6] for entry in entries) \
                         / len(entries)
        avg_grid_usage_cost = sum(entry[8] for entry in entries) \
                              / len(entries)
        avg_total_savings = (avg_energy_usage - avg_grid_usage) \
                            * GRID_COST_PER_KWH
...
def generate_yearly_report():
    ...
    for entry in yearly_data:
        month = entry[0].replace(day=1, hour=0, minute=0,
...
# Create a frame for the report text field
report_frame = tk.LabelFrame(main_frame,
                             text="Report Output",
                             padx=10,
                             pady=10,
                             font=("Arial", 12, "bold"))
report_frame.grid(row=7,
                  column=0,
                  columnspan=3,
                  padx=10,
                  pady=10,
...
# Add a text field to display the report with a scrollbar
